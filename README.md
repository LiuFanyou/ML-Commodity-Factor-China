...
信号在决策日 `t` 收盘后形成，`t+1` 开盘执行。主标签为从 `t+1` 开盘开始的未来 20 个 open-to-open 收益几何累计值，并在执行日进行截面去均值。方向头预测超额收益是否大于零，回归头预测未来 20 日超额收益幅度。
...
模型采用扩展窗口验证。首个训练窗为 2015—2021 年，2022 年样本外测试；后续逐年扩展。未来 20 日标签存在重叠，因此训练期每 5 个交易日保留一个日期的完整品种截面，验证和测试仍逐日预测。训练/验证之间保留 40 日 purge，并要求训练标签的退出日早于测试年开始。

## 组合与风控

回测沿用双头模型和 GMM 风控，但采用降频执行。预测信号先做 3 日 EMA；多头在当日信号上 30%且大于零时进入，空头在下 30%且小于零时进入。已有多头跌破 40%分位才退出，已有空头升破 60%分位才退出。决策日流动性最低 20%的品种禁止新开仓，但已有持仓仍按退出线正常处理。组合按预测强度加权，并使用仅由历史数据拟合的两状态 GMM；高压力状态下组合毛杠杆降至 50%。
